...
namespace Projeto_IB1
{
    public partial class Form1 : Form
    {
        ...
        Thread thrAq, thrPlot, thrChoque;

        bool isEstimulando = false;
        ...
        public Form1()
        {
            ...
            //thrChoque = new Thread(aplicar_choque);
            //thrChoque.Priority = ThreadPriority.Normal;
            ...
            while (flagLife)
            {
                while (runThAq)
                {
                    ...
                    if(serial.BytesToRead > 0)
                    {
                        ...
                        int amp = int.Parse(input);                        ampEscala = ((amp * 5.0) / 1000);
                        setText_label_amp(Math.Round(ampEscala, 3).ToString()); // arredondamento para 3 casas decimais
                        y = new double[2];
                        y[0] = ampEscala;
                        y[1] = contSerial;
        ...
        private void btEstimular_Click(object sender, EventArgs e)
        {
            if (!serial.IsOpen) //lbTempo q ser a serial do tensy, so uma serial
            {
                Debug.Write("PORTA SERIAL NAO ABERTA. ABRIR PRIMEIRO, DEPOIS ENVIAR O COMANDO");
                // abrindo a porta serial
                try
                {
                    serial.Open();
                    // enviando o comando apos abrir a porta serial
                    //serial.Write("A");
                }catch(Exception exc)
                {
                    Console.WriteLine(exc);
                    Debug.Write("Erro no programa!");
                }
            }
            else
            {
                Debug.Write("PORTA SERIAL ABERTA. PODEMOS ENVIAR O COMANDO");
                if(isEstimulando)
                {
                    serial.WriteLine("B");
                    isEstimulando = false;
        ...
        {
            Console.Write("ESTOU EM INICIAR");
            ...
            serial.BaudRate = 115200; //estabelece o baud rate da comunicação (115200)
        ...
        //private void aplicar_choque()
        //{
        //    double tensao_maxima = 01010101010101;
        //    serial.Write("choque");
        //    TimeSpan inicio = new TimeSpan();
        //    if(ampEscala >= tensao_maxima)
        //    {
        //        TimeSpan final = new TimeSpan();
        //        TimeSpan total = final - inicio;
        //        lbTime.Text = Convert.ToString(total);
        //        lbTime.Visible = true;
        //        //colocar label para mostrar o tempo, fazer funcao de conversao de segundos fracionados
        //    }
        //}
    }
}
